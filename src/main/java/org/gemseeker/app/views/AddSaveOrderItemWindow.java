 ...
 * @author Gem
 */
public class AddSaveOrderItemWindow extends AbstractWindowController {
    ...
    public AddSaveOrderItemWindow(EditOrderWindow editOrderWindow) {
        super("Add Order Item", AddOrderItemWindow.class.getResource("add_order_item.fxml"), editOrderWindow.getWindow());
    ...
    private void saveAndClose() {
        ...
        disposables.add(Single.fromCallable(() -> {
            ...
            orderItem.setDate(mOrder.getDate());
            ...
            if(added) {
                ...
                ShipperStock shipperStock = database.getShipperStock(mOrder.getShipper().getId(), orderItem.getProductId());
                boolean success;
                if (shipperStock == null) {
                    shipperStock = new ShipperStock();
                    shipperStock.setShipperId(mOrder.getShipper().getId());
                    shipperStock.setProductId(orderItem.getProductId());
                    shipperStock.setInStock(orderItem.getQuantity());
                    shipperStock.setDelivered(0);
                    shipperStock.setSales(0);
                    success = database.addEntry(shipperStock);
                } else {
                    int newInStock = shipperStock.getInStock() + orderItem.getQuantity();
                ...
                // update Stocks
                if (success) {
                    Stock stock = database.getStock(orderItem.getProductId());
                    if (stock != null) {
                        int qtyOut = stock.getQuantityOut() + orderItem.getQuantity();
                        database.updateEntry("stocks", "quantity_out", qtyOut, "id", stock.getId());

                        int inStock = stock.getInStock() - orderItem.getQuantity();
            ...
            showProgress(false);
            if (!success) showInfoDialog("Failed to add order item.", "");
            close();
            editOrderWindow.refresh();
        }, err -> {
            showProgress(false);
            showErrorDialog("Database Error", "Error while adding order item.", err);
        }));
    }

    public void show(Order order) {
        if (order == null) {
            showInfoDialog("Invalid", "No order selected.");
            return;
        }
        mOrder = order;
        super.show();
