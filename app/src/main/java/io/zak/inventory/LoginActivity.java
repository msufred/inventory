...
public class LoginActivity extends AppCompatActivity {
    ...
    private void login() {
        ...
        Log.d(TAG, "Validating login credentials: " + validated);

        if (validated) {
            // find User
            progress.setVisibility(View.VISIBLE);
            disposables.add(Single.fromCallable(() -> {
                Log.d(TAG, String.format("Find User (%s, %s): %s", username, password, Thread.currentThread()));
                return AppDatabaseImpl.getDatabase(getApplicationContext()).users().getUser(username, password);
            }).observeOn(AndroidSchedulers.mainThread()).subscribeOn(Schedulers.io()).subscribe(users -> {
                progress.setVisibility(View.INVISIBLE);
                Log.d(TAG, String.format("DONE\nReturned list with size:%d %s", users.size(), Thread.currentThread()));
